...
digits = (parseInt(digit) for digit in rawDigits.split(" "))
grid = Array(20)
for i in [0...20]
  grid[i] = Array(20)
...
getLineProduct = (sx, sy, dx, dy) ->
  ex = sx + (4 * dx)
  return -1 if (ex < 0) or (ex >= 20)
  ey = sy + (4 * dy)
  return -1 if (ey < 0) or (ey >= 20)
...
getLine = (sx, sy, dx, dy) ->
  ex = sx + (4 * dx)
  return [] if (ex < 0) or (ex >= 20)
  ey = sy + (4 * dy)
  return [] if (ey < 0) or (ey >= 20)
...
problem.run =
  test: ->
    ...
    ok(true, JSON.stringify(getLine(4, 8, 1, 0)))
  ...
  answer: ->
    ...
    for j in [0...20]
      for i in [0...20]
        # maxProduct = Math.max(maxProduct, getLineProduct(i, j, 1, 0))
        # maxProduct = Math.max(maxProduct, getLineProduct(i, j, 0, 1))
        # maxProduct = Math.max(maxProduct, getLineProduct(i, j, 1, 1))
